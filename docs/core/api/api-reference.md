...
- Core Supervisor fleet/runtime:
  ...
  - Core refreshes stored Supervisor version audit metadata on startup and every 10 minutes by default; `metadata.version_audit` is advisory and separate from freshness. `metadata.local_source_gate` records whether the Core-host Supervisor package source is safe to use for later remote auto-update decisions.
