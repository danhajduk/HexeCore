...
- Events/services:
  ...
  - `GET /api/system/nodes/budgets/policy/current?node_id=...` (trusted node token required via `X-Node-Trust-Token`; current effective budget policy and grants)
  - `POST /api/system/nodes/budgets/policy/refresh` (trusted node token required via `X-Node-Trust-Token`; version-aware budget policy refresh)
  ...
  - `POST /api/system/nodes/services/resolve` (trusted node token required via `X-Node-Trust-Token`; node-aware task-family to service/provider resolution using governance and budget policy)
  - `POST /api/system/nodes/services/authorize` (trusted node token required via `X-Node-Trust-Token`; short-lived service token issuance gated by resolution and effective budget)
  ...
  - `GET /api/system/nodes/governance/current?node_id=...` (trusted node token required via `X-Node-Trust-Token`; governance bundle now includes routing-policy constraints and budget policy when available)
  - `POST /api/system/nodes/governance/refresh` (trusted node token required; version-aware governance refresh across capability, routing-policy, and budget-policy changes)
