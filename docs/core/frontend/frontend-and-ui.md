...
- Addon and node proxy flows emit structured `hexe.proxy` logs for UI, API, and websocket surfaces, including target id, public prefix, latency, status, and failure outcome for operator debugging.
