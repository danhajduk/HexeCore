...
- Node routes: `/nodes/:nodeId` for registry details and `/nodes/:nodeId/UI` for the Core-proxied node UI iframe when the node registration exposes `ui_enabled=true` and a canonical `ui_base_url`.
