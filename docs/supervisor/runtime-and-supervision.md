...
- Registered Node runtime summaries and Core runtime summaries record Supervisor-observed aggregate resource samples into the same history store. Runtime and child samples persist CPU, memory, RSS, process/container identity, and service telemetry fields such as `rps`, `latency_ms_avg`, `latency_ms_p95`, `error_rate`, and `inflight` when reported.
- For the local Core-attached Supervisor, Core enriches the `core-api` runtime record with the current API metrics snapshot before the Supervisor Fleet registry stores the heartbeat, so the Core Services and Aux Runtimes table values are retained in the fleet record and history samples instead of existing only in the browser view.
- Nested `runtime_metadata.services` and `runtime_metadata.containers` entries with observed process/container metrics are also persisted as child resource samples linked to the parent runtime id.
...
- Core-hosted runtime history can be read with `GET /api/supervisor/core/runtimes/{runtime_id}/resources/history?range=24h&step=60s`. The response uses `scope=core_runtime` and includes aggregate samples, lifecycle events, and child service/container samples when present.
...
- Core proxies local configured Supervisor history through `GET /api/system/supervisor/core/runtimes/{runtime_id}/resources/history` for Core-hosted runtimes.
- Core proxies fleet-scoped local or remote Supervisor history through `GET /api/system/supervisors/{supervisor_id}/resources/history`, `GET /api/system/supervisors/{supervisor_id}/runtimes/{node_id}/resources/history`, and `GET /api/system/supervisors/{supervisor_id}/core/runtimes/{runtime_id}/resources/history`, using the local Supervisor client for attached Supervisors and the registered `api_base_url` for remote Supervisors.
- Core proxies fleet-scoped Supervisor update control through `GET /api/system/supervisors/{supervisor_id}/update/status` and `POST /api/system/supervisors/{supervisor_id}/update/start`. Core only forwards update requests to Supervisors that are currently `online`, advertise the requested update mode, and expose a local or registered remote Supervisor API. `git` mode delegates to the Supervisor-local updater unit; `core_host` mode builds a checksum-validated package from Core's local Supervisor source and lets the target Supervisor stage, back up, and apply it inside its own install root.
- Core Settings renders local and remote Supervisor resource history in the Supervisor page so operators can correlate recent host pressure, Node runtime CPU/memory trends, Core/Aux runtime RPS/p95/error trends, and lifecycle markers before exits or restarts. The default fleet list hides long-offline remote Supervisor records after `HEXE_SUPERVISOR_FLEET_HISTORICAL_S`; historical records remain available through `GET /api/system/supervisors?include_historical=true`.
- Core Settings also renders Supervisor update version, supported modes, last update state/time/error, and update actions for online Supervisors. `git` and `core_host` are offered only when the target Supervisor reports support, and each action requires an operator confirmation naming the Supervisor, host, and source mode.
- Legacy local Supervisor records are also moved to historical visibility when they are offline and a newer local-attached Supervisor record for the same host is online. This keeps stale pre-migration rows such as an old `Hexe` supervisor id out of the default fleet without deleting the audit trail.
- Supervisor Fleet API rows include `freshness_age_s`, `freshness_reason`, `visibility_state`, and `visibility_reason` so the UI can distinguish healthy-but-stale state from a current healthy heartbeat. Core Settings renders these as freshness labels, a diagnostic column, and the count of hidden historical records.
...
- Embedded MQTT docker runtime restart policy is owned by runtime boundary config (`backend/app/system/mqtt/runtime_boundary.py`) via:
  - `HEXE_MQTT_DOCKER_RESTART_POLICY` (default `no`)
