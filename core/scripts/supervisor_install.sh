...
BLUETOOTH_ACCESS_POLICY="$(hexe_env HEXE_BLUETOOTH_ACCESS_POLICY)"
BLUETOOTH_ENSURE_POWERED="$(hexe_env HEXE_BLUETOOTH_ENSURE_POWERED)"
BLUETOOTH_POWER_RETRY_S="$(hexe_env HEXE_BLUETOOTH_POWER_RETRY_S)"
HARDWARE_LEASE_SECRET="$(hexe_env HEXE_HARDWARE_LEASE_SECRET)"
HARDWARE_LEASE_VALIDATE_URL="$(hexe_env HEXE_HARDWARE_LEASE_VALIDATE_URL)"
...
write_supervisor_env() {
  if [[ -z "$INSTALL_MODE" && -z "$CORE_URL" && -z "$SUPERVISOR_ID" && -z "$SUPERVISOR_NAME" && -z "$SUPERVISOR_PUBLIC_URL" && -z "$BLUETOOTH_ACCESS_POLICY" && -z "$BLUETOOTH_ENSURE_POWERED" && -z "$BLUETOOTH_POWER_RETRY_S" && -z "$HARDWARE_LEASE_SECRET" && -z "$HARDWARE_LEASE_VALIDATE_URL" ]]; then
    return 0
  fi
  ...
  local env_file="$HOME/.config/hexe/supervisor.env"
  {
    ...
    write_env_if_set "HEXE_BLUETOOTH_ACCESS_POLICY" "$BLUETOOTH_ACCESS_POLICY"
    write_env_if_set "HEXE_BLUETOOTH_ENSURE_POWERED" "$BLUETOOTH_ENSURE_POWERED"
    write_env_if_set "HEXE_BLUETOOTH_POWER_RETRY_S" "$BLUETOOTH_POWER_RETRY_S"
    write_env_if_set "HEXE_HARDWARE_LEASE_SECRET" "$HARDWARE_LEASE_SECRET"
    write_env_if_set "HEXE_HARDWARE_LEASE_VALIDATE_URL" "$HARDWARE_LEASE_VALIDATE_URL"
    if [[ -n "$CORE_URL" ]]; then
