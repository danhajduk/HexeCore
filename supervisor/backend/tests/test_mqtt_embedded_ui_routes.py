...
class TestMqttEmbeddedUiRoutes(unittest.TestCase):
    ...
    def test_root_ui_page_contains_setup_shell(self) -> None:
        ...
        self.assertIn("Approve MQTT Addon", res.text)
        self.assertIn("Provision MQTT Addon", res.text)
        self.assertIn('data-section="bridge-grants"', res.text)
        self.assertIn("Bridge Grants", res.text)
        self.assertIn("/api/system/mqtt/node-bridge-grants", res.text)
        self.assertIn("data-bridge-grant-action='approve'", res.text)
        self.assertIn("data-registration-action='approve'", res.text)
        self.assertIn("data-registration-action='provision'", res.text)
        self.assertIn("/api/system/mqtt/registrations/approve", res.text)
