...
class TestModelRoutingRegistry(unittest.TestCase):
    def test_records_and_groups_provider_model_metadata(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
                        ...
                        "available_models": [
                            {
                                ...
                                "capacity": {"period": "daily", "limits": {"max_requests": 2500}},
                            }
                        ],
                    }
                ],
            ...
            self.assertEqual(records[0].service_capacity["service"], "ai.inference")
            self.assertEqual(records[0].provider_capacity["limits"]["max_tokens"], 500000)
            self.assertEqual(records[0].model_capacity["limits"]["max_requests"], 2500)
            ...
            self.assertEqual(grouped[0]["service_capacity"]["limits"]["max_cost_cents"], 1000)
            self.assertEqual(grouped[0]["providers"][0]["provider"], "openai")
            self.assertEqual(grouped[0]["providers"][0]["capacity"]["limits"]["max_tokens"], 500000)
