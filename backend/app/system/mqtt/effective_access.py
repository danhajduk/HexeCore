...
from .topic_families import (
    ...
    core_runtime_notify_write_topics,
...
class MqttEffectiveAccessCompiler:
    ...
    def _from_principal(self, principal: MqttPrincipal, state: MqttIntegrationState) -> MqttEffectiveAccessEntry | None:
        ...
        if principal.principal_type == "synthia_addon":
            ...
            publish_topics = _sorted_unique(list(principal.publish_topics))
            subscribe_topics = _sorted_unique(list(principal.subscribe_topics))
            if principal.principal_id == "core.runtime":
                publish_topics = _sorted_unique([*publish_topics, *self._core_runtime_notify_write_topics])
            reserved_denies = []
        else:
            return None
