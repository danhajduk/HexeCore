...
class TestMqttStartupReconcile(unittest.TestCase):
    def test_reconcile_success_marks_ready(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            onboarding_endpoints = dict(bootstrap_payload.get("onboarding_endpoints") or {})
            self.assertEqual(onboarding_endpoints.get("register_session"), "/api/system/nodes/onboarding/sessions")
            self.assertEqual(onboarding_endpoints.get("registrations"), "/api/system/nodes/registrations")
            self.assertEqual(onboarding_endpoints.get("register"), "/api/system/nodes/onboarding/sessions")
            self.assertEqual(onboarding_endpoints.get("ai_node_register"), "/api/system/ai-nodes/onboarding/sessions")
            self.assertEqual(bootstrap_payload.get("onboarding_mode"), "api")
            self.assertEqual(bootstrap_payload.get("onboarding_contract"), "global-node-v1")
            compatibility = dict(bootstrap_payload.get("compatibility") or {})
            self.assertEqual(compatibility.get("legacy_ai_node_keys"), ["register", "ai_node_register"])
            self.assertTrue(str(bootstrap_payload.get("emitted_at") or "").strip())
            self.assertNotIn("setup_state", onboarding_endpoints)
